...
def main(argv):
    ...
    with open(data_dir+'/classic_obj.pkl','r') as file:
        obj = pickle.load(file)
    true_obj = obj[env_name][idx[2]]

    filename = save_dir + 'dice-mujoco-' + str(env) + '-discount-' + str(gamma) \
    ...
    for seed in seeds:
        for size in size_lists:
            ...
            dataset = load_dataset(args.data_dir,'/dataset/'+ name, size,max_trajectory_length,env_name,
            ...
            dataset2 = load_dataset(args.data_dir,'/dataset_test/'+name, size, max_trajectory_length, env_name,
                                   action_discrete=False)
            ...
            name = ['discount_factor', gamma, 'random_weight', random_weight, 'max_length', max_trajectory_length,
                    'buffer_size', size, 'seed', seed, 'env', env_name]
            name = '-'.join(str(x) for x in name)
                ...
                if step % 5 == 0 or step == num_steps - 1:
                    ...
                    if (eval_obj2-true_obj)**2 < best:
                        ...
                        if save_dir is not None:
                            model_save_path = os.path.join(save_dir, name+'_best_model_weights')
            ...
            # Save the model at the end of training
            if save_dir is not None:
                model_save_path = os.path.join(save_dir, name+'_last_model_weights')
