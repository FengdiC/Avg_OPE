...
class NNGammaCritic(nn.Module):
    ...
    def forward(self, obs):
        ...
        weight = self.weight(body.detach())
...
class MLPWeightedActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, hidden_sizes=(64,64),
                 critic_hidden_sizes=(4000,4000),activation=nn.Tanh):
        super().__init__()
        obs_dim = observation_space.shape[0]
        ...
        # build value function
        self.v = NNGammaCritic(obs_dim, critic_hidden_sizes,activation=nn.ReLU)
...
class MLPSeparateWeightedActorCritic(nn.Module):
    def __init__(self, observation_space, action_space,
        ...
        # build value function
        self.v  = MLPCritic(obs_dim, hidden_sizes, activation)
        self.w = MLPGamma(obs_dim, critic_hidden_sizes, activation)
