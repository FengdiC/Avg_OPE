...
class PPOBuffer:
    ...
    def sample(self,batch_size,fold_num):
        ...
        data = dict(obs=self.obs_buf[ind], act=self.act_buf[ind], prod=self.prod_buf[ind],
                    next_obs=self.next_obs_buf[ind], tim=self.tim_buf[ind],
...
def train(lr, env,seed,path,hyper_choice,link,random_weight,l1_lambda, discount=0.95,
          checkpoint=5,epoch=1000,cv_fold=1,batch_size=256,buffer_size=20,max_len=50, **kwargs):
    ...
    def update(fold_num):
        ...
        with torch.no_grad():
            label = torch.exp(np.log(gamma) + (logtarg - logbev) + weight(obs)) + (1 - gamma)
