...
class PPOBuffer:
    ...
    def store(self, obs, act, rew, done, tim, logbev, logtarg):
        """
    ...
    def sample(self,batch_size,fold_num):
        ...
        data = dict(obs=self.obs_buf[ind], act=self.act_buf[ind], prod=self.prod_buf[ind],
                    tim=self.tim_buf[ind], done=self.done_buf[ind],
                    logbev=self.logbev_buf[ind], logtarg=self.logtarg_buf[ind])
        return {k: torch.as_tensor(v, dtype=torch.float32) for k, v in data.items()}

    def sample_nstep(self, batch_size, n):
        ind = np.random.randint(self.ptr, size=batch_size)        
        dones = np.where(self.done_buf == 1)[0]

        # Nth step index
        ind_nstep = []
        for k, index in enumerate(ind):
            ind_nstep.append(min(index + n, dones[k]))
        ind_nstep = np.array(ind_nstep)

        # IS sampling correction term
        corr_term = []
        for i, j in zip(ind, ind_nstep):
            x = 0
            for k in range(i, j):
                x += (np.log(self.gamma) * self.tim_buf[k] + self.prod_buf[k])
            corr_term.append(x)
        corr_term = np.array(corr_term)

        data = dict(obs=self.obs_buf[ind], act=self.act_buf[ind], prod=self.prod_buf[ind],
                    tim=self.tim_buf[ind], done=self.done_buf[ind],
                    logbev=self.logbev_buf[ind], logtarg=self.logtarg_buf[ind],
                    nstep_obs=self.obs_buf[ind_nstep], corr_term=corr_term)
        return {k: torch.as_tensor(v, dtype=torch.float32) for k, v in data.items()}
...
def collect_dataset(env,gamma,buffer_size=20,max_len=200,
    ...
    while num_traj < buffer_size:
        ...
        # save and log
        buf.store(o, a, r, ep_len - 1, d, logbev, logtarg)
...
def train(lr, env,seed,path,hyper_choice,link,random_weight,l1_lambda, discount=0.95,
    ...
    def update(fold_num):
        #sample minibatches
        data = buf.sample_nstep(batch_size, 1)

        obs, act = data['obs'], data['act']
        tim, prod = data['tim'], data['prod']
        nstep_obs, corr_term = data['nstep_obs'], data['corr_term']

        loss = (corr_term*torch.exp(weight(obs).detach()) + 1 - torch.exp(weight(nstep_obs)) ** 2).mean()
...
if __name__ == "__main__":
    ...
    objs = train(0.001,env='CartPole-v1',seed=2,
                 path='./exper/cartpole.pth',hyper_choice=394,
